...
class MLPModel(BenchmarkModel):
    ...
    def hyperopt(self, data: BenchmarkData, operator: OperatorInfo) -> dict[str, Any]:
        ...
        def objective(trial):
            ...
            model = DirectODRegressor(data.n_features, data.n_targets, cfg, residual_blocks=False)
    ...
    def fit(
        ...
        *,
        ...
        self.torch_model = DirectODRegressor(
            data.n_features, data.n_targets, self.cfg, residual_blocks=False
        )
        ...
        torch.save(
            {"state_dict": self.torch_model.state_dict(), "params": params, "cfg": self.cfg},
            path,
        )
        result = FitResult(
            ...
            notes="Direct MLP; constraints via output_activation; no silent clip",
        )
