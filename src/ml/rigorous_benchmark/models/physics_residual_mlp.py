"""Physics Residual MLP — residual blocks with stronger forward loss weight."""
...
class PhysicsResidualMLPModel(BenchmarkModel):
    name = "physics_residual_mlp"
    ...
    def hyperopt(self, data: BenchmarkData, operator: OperatorInfo) -> dict[str, Any]:
        ...
        def objective(trial):
            ...
            del p["forward_weight"]
            p["forward_weight"] = trial.suggest_float("physics_forward_weight", 0.5, 3.0)
            ...
            pred = predict_regressor(model, x_va, bundle, cfg, residual_learning=True)
            del model
        ...
        if "physics_forward_weight" in params:
            params["forward_weight"] = params.pop("physics_forward_weight")
    ...
    def fit(
        ...
        *,
        ...
        result = FitResult(
            ...
            notes="Residual MLP + residual learning from base OD + elevated forward loss",
        )
    ...
    def predict(
        ...
        return predict_regressor(
            self.torch_model, xs, self.bundle, self.cfg, residual_learning=True
        )
